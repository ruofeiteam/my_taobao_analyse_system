...
def create_word_bigram_scores():
    ...
    for word, freq in word_fd.items():
        pos_score = BigramAssocMeasures.chi_sq(cond_word_fd['pos'][word], (
            freq, pos_word_count), total_word_count)  # 计算积极词的卡方统计量，这里也可以计算互信息等其它统计量
...
def compare_test():
    ...
    global pos_review, neg_review
    classifiers = ['BernoulliNB', 'MultinomiaNB',
                   'LogisticRegression', 'SVC', 'LinearSVC', 'NuSVC']
    ...
    dimension = [500, 1000, 1500, 2000, 2500, 3000, 3500, 4000,
...
def transfer_text_to_moto():
    ...
    moto = pickle.load(
        open('../pkl_data/test/test_review.pkl', 'rb'))  # 载入文本数据
